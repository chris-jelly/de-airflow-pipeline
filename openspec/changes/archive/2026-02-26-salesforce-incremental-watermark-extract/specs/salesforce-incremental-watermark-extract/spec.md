...
The extraction process SHALL query `Account`, `Contact`, and `Opportunity` using explicit curated field lists and SHALL NOT use wildcard selection.

#### Scenario: Query uses explicit fields
- **WHEN** the DAG builds a SOQL query for an object
- **THEN** it SHALL generate `SELECT <explicit field list> FROM <object>` and SHALL NOT generate `SELECT *`

#### Scenario: Curated list includes change-tracking fields
